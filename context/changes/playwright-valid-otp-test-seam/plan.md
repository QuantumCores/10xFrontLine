...
- [x] 1.1 Captured store tests pass: `dotnet test src/api/frontLineApi.slnx --filter FullyQualifiedName~CapturedEmailStoreTests` — 53e1450
- [x] 1.2 API solution builds cleanly: `dotnet build src/api/frontLineApi.slnx` — 53e1450
...
- [x] 3.1 Full API test suite passes: `dotnet test src/api/frontLineApi.slnx` — 636e7fa
- [x] 3.2 API solution builds cleanly: `dotnet build src/api/frontLineApi.slnx` — 636e7fa
- [x] 3.3 Generated CLI output is ignored: `git check-ignore .playwright-cli/page-generated.yml` — 636e7fa

#### Manual

- [x] 3.4 A human starts the API in exact E2E on `127.0.0.1`, follows the documented request/retrieve/fill/verify sequence, and reaches the authenticated `/play` route — 636e7fa
- [x] 3.5 A second retrieval for the same issued code returns not found, and requesting a new code restores the workflow — 636e7fa
- [x] 3.6 Starting the API normally in Development leaves the E2E route unavailable while preserving the existing Development OTP console log — 636e7fa
- [x] 3.7 Local `.playwright-cli/` snapshots continue to appear and provide current element refs without showing as untracked Git files — 636e7fa
