...
- [ ] 2.1 Auth integration tests pass: `dotnet test src/api/frontLineApi.slnx --filter Auth`
- [ ] 2.2 API builds cleanly: `dotnet build src/api/frontLineApi.slnx`

#### Manual

- [ ] 2.3 A developer can request a code locally without configuring Gmail
- [ ] 2.4 Auth endpoints never reveal whether an email already had an account

### Phase 3: Completed Result Write Contract

#### Automated

- [ ] 3.1 Result integration tests pass: `dotnet test src/api/frontLineApi.slnx --filter Results`
- [ ] 3.2 Full API tests pass: `dotnet test src/api/frontLineApi.slnx`
- [ ] 3.3 API builds cleanly: `dotnet build src/api/frontLineApi.slnx`

#### Manual

- [ ] 3.4 A locally issued JWT can save a result through the HTTP example or API client
- [ ] 3.5 Re-sending the same completed result does not create a duplicate row

### Phase 4: Thin Angular Auth and Result Client

#### Automated

- [ ] 4.1 Angular production build passes: `npm run build`
- [ ] 4.2 Angular tests pass: `npm test`
- [ ] 4.3 Client auth/result specs pass through the configured test target

#### Manual

- [ ] 4.4 Browser flow can request a code, verify it using the dev/test delivery path, persist the JWT, and reach a protected placeholder route
- [ ] 4.5 Authenticated result client can submit to the local API when supplied a completed-result summary
