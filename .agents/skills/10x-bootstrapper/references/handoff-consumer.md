...
- `subdir-then-move` and `git-clone` strategies: `{name}` becomes the resolved `scaffold_name` from `bootstrapper-config.yaml`, defaulting to `.bootstrap-scaffold`. A `scaffold_name` value may contain `{project_name}`, which is resolved from the hand-off before `{name}` substitution. The CLI scaffolds into that temp directory and bootstrapper moves files up afterward.
