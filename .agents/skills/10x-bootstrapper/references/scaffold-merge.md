...
- `scaffold_name` from `bootstrapper-config.yaml` for this `starter_id`, defaulting to `.bootstrap-scaffold` if the id or field is not listed. Resolve a `{project_name}` token from the hand-off before using it.

## Substitution rules

Two placeholders, substituted before exec:

- `{name}` — replaced by the resolved `scaffold_name` for `subdir-then-move` and `git-clone`; replaced by `.` for `native-cwd`.
...
1. Substitute `{name}=<scaffold_name>` (and `{pm}` if present) into `cmd_template`.
...
4. Apply the conflict matrix below to move files from `<scaffold_name>/` up into cwd.
5. Delete `<scaffold_name>/` (the directory should be empty after the move; if it is not, log the leftover paths into the verification log and remove the directory anyway — the move-up step is best-effort, not transactional).
...
2. Run the resolved command. Capture stdout, stderr, and exit code.
3. If exit code != 0, run the CLI failure handling below. Stop here on failure.
4. **Delete `<scaffold_name>/.git/`** before the move-up. The cloned `.git/` carries the upstream starter's history, which is rarely what the user wants; the user typically initialises their own repo afterwards. If cwd already contains a `.git/`, the existing-wins rule keeps it untouched.
5. Apply the conflict matrix below to move files up into cwd.
6. Delete `<scaffold_name>/`.

## Conflict matrix

Applied during the move-up step for `subdir-then-move` and `git-clone`. Compares each path under `<scaffold_name>/` against the same relative path under cwd.
...
Any `starter_id` not listed in `bootstrapper-config.yaml`'s `starters:` map defaults to `subdir-then-move` with `scaffold_name: .bootstrap-scaffold`. This is the safe default — the CLI scaffolds into a temp dir, then the conflict matrix governs the move-up, and cwd state (including `context/`) is preserved. A starter that rejects dot-prefixed project names or embeds the CLI name into generated metadata must declare a CLI-valid `scaffold_name` override, typically `{project_name}`.

A starter that is genuinely cwd-aware (its CLI was designed to write into `.`) but is not yet listed in `bootstrapper-config.yaml` will scaffold sub-optimally under `subdir-then-move` — the CLI may write into the resolved temporary scaffold directory, the move-up still works correctly, but the user pays an extra copy/delete round-trip. Worked example: `fastapi` (hypothetical card not in v1 config) — `cmd_template` `pip install fastapi[standard] && fastapi run app/main.py` is run in the resolved temporary directory; it works, but a future bootstrapper-config entry pinning `fastapi: cwd_strategy: native-cwd` would avoid the temp-dir round-trip.
...
2. Leave `<scaffold_name>/` in place for the user to inspect (do NOT delete it on failure — the user may want to read partial output, log files, or rerun manually).
...
6. Print verbatim (substitute `<exit_code>` and `<short_stderr>`):
   ...
   ```
   Scaffold CLI exited with status <exit_code>. The scaffold directory `<scaffold_name>/` is left in place for inspection. Last stderr line:
